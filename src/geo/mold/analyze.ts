...
/** Порог, ниже которого зацепы считаем несущественными: гипс их прощает. */
const DROPOUT_TOLERANCE = 0.002;
const HALVES_TOLERANCE = 0.005;
/**
...
export function analyzeMold(
  ...
  if (halves.fraction > HALVES_TOLERANCE) {
    warnings.push(
      `Поднутрения у плоскости разъёма: ${(halves.fraction * 100).toFixed(1)} % поверхности. ` +
      'Такой узор форма не отпустит — уменьшите глубину накатки или разверните её.',
    );
  }
