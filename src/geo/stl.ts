...
//
// Ядро ClayForm считает в миллиметрах, поэтому scale по умолчанию 1 — в
// отличие от mathsculpt, где ядро работало в относительных единицах.

import type { SurfaceMesh } from './surface';

export interface StlOptions {
  /** множитель модель→мм; ядро уже в мм, поэтому по умолчанию 1 */
  scale?: number;
  /** попадает в 80-байтовый заголовок вместе с брендом */
  name?: string;
}
...
export function encodeSTL(
  ...
  const header = `ClayForm ${o.name ?? ''}`.slice(0, 80);
